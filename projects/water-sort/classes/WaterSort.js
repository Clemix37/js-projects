...
export default class WaterSort {
	...
	get nbOfTubes() {
		return this.#nbOfTubes;
	}
	get heightOfTube() {
		return this.#heightOfTube;
	}
	get tubes() {
		return this.#tubes;
	}
	...
	#selectTargetTube(targetId) {
		...
		const canMoveColor =
			targetTubeGen.slots.length < this.#heightOfTube &&
			(topColorTarget === topColorSelected ||
				(!topColorTarget && topColorSelected) ||
				(!topColorSelected && topColorTarget));
		if (canMoveColor) {
